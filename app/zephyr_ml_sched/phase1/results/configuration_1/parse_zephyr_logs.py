...
ML_PAT = re.compile(r"\[ML\] run=(\d+) cycles=(\d+)")
...
def parse_one_file(path: Path) -> dict:
    ...
    ml_df = pd.DataFrame(ml, columns=["run", "cycles"]) if ml else pd.DataFrame(columns=["run", "cycles"])
    ...
    return {
        ...
        "avg_ml_cycles": ml_df["cycles"].mean() if not ml_df.empty else float("nan"),
        "max_ml_cycles": ml_df["cycles"].max() if not ml_df.empty else float("nan"),
